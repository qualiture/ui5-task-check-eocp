...
module.exports = async ({ log, taskUtil }) => {
                ...
                .then((result) => {
                    ...
                    if (patch) {
                        if (isBeforeQuarter(patch.eocp)) {    
                            ...
                            log.error(`\u001b[41m\u001b[1m⚠️  WARNING! Your UI5 version '${patch.version}' has reached End of Cloud Provisioning (${patch.eocp})! Update as soon as possible!\u001b[0m`);
                            log.error(`\u001b[41m\u001b[1m            Visit \u001b[41m\u001b[3mhttps://ui5.sap.com/versionoverview.html\u001b[0m\u001b[41m\u001b[1m for applicable versions\u001b[0m`);
                        }
                    }
                })
                .catch((err) => {
                    log.error('Error fetching UI5 version overview. Task ui5-task-check-eocp will be skipped.');
                });
        } else {
            log.warn("No UI5 version specified in ui5.yaml file! Cannot determine End of Cloud Provisioning date!")
        }
    }
    ...
    function findMatchingPatch(checkVersion, patches) {
        ...
        if (!patch) {
            ...
            log.warn(`Your UI5 version '${checkVersion}' does not exist! The closest available match to your version would be '${closest}' (see: https://ui5.sap.com/versionoverview.html)`);
        } else if (!patch.eocp) {
