...
module.exports = async ({ log, taskUtil, options }) => {
        ...
        if (currentVersion) {
            fetch('https://ui5.sap.com/versionoverview.json')
                ...
                .then((result) => {
                    ...
                    checkLTS(currentVersion, result);
                })
                .catch((err) => {
    ...
    /**
     * Checks if a new LTS version is available
     * @param {*} currentVersion 
     * @param {*} result 
     */
    function checkLTS(currentVersion, result) {
        const latestLTS = result.versions.find(elem => elem.lts === true);
        if (latestLTS) {
            if (semver.satisfies(currentVersion, latestLTS.version)) {
                log.info(`\u001b[32m\u001b[1mYou are on the latest LTS version\u001b[0m`);
            } else if (semver.ltr(currentVersion, latestLTS.version)) {
                log.info(`\u001b[32m\u001b[1m💡 A new LTS version of UI5 is available: '${latestLTS.version}'!\u001b[0m`);
            } else {
                log.info(`\u001b[33m\u001b[1m💡 You are ahead of the latest LTS version '${latestLTS ? latestLTS.version : 'none'}'\u001b[0m`);
            }
        } 
    }
